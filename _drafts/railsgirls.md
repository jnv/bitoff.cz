...
Před dvěma týdny jsem měl možnost koučovat na třetím ročníku [Rails Girls v Praze](http://railsgirls.com/prague). Byla to natolik výborná zkušenost, že jsem se rozhodl sepsat k ní pár myšlenek -- a zároveň jsem chtěl reflektovat připomínky, které jsem probíral s účastnicemi a ostatními kouči.

Pokud nevíte, co jsou [Rails Girls](http://railsgirls.com/) zač, jedná se o komunitu s cílem zpřístupnit programování ženám a dívkám. Začalo to v roce 2010 ve Finsku víkendovými workshopy, které se dnes [konají po celém světě](http://railsgirls.com/events). Od minulého roku také pořádají [Rails Girls Summer of Code](http://railsgirlssummerofcode.org/), kde účastnice tři měsíce pracují na vybraných open-source projektech -- a dostanou za to zaplaceno. Je to vlastně ekvivalent [Google Summer of Code](http://www.google-melange.com/) pro začínající programátorky.

Kromě Prahy se pořádají workshopy také [v Ostravě](http://railsgirls.com/ostrava) a (zrovna tento víkend) [v Brně](http://railsgirls.com/brno) -- zaslechl jsem něco i o Olomouci.

Workshopy probíhají zhruba takto:

* V pátek je možné přijít na „install party,“ kde se vyřeší základní nástroje: Ruby, Rails a textový editor.
...
Na jeden víkend je toho skutečně hodně avšak cílem akce není z účastnic udělat ostřílené programátorky během tří dnů. Cílem je ukázat, že počítače nejsou černé skřínky žijící si vlastním životem a programování není magie, kterou by měla ovládat úzká skupina vyvolených. Řada účastnic má „ajťáky“ ve svém okolí a díky Rails Girls nahlédnou do kryptického žargonu, na kterém celé IT stojí. Zároveň získají *nějakou* představu, jak fungují webové stránky které denně používají, ať už je to iDnes, IDOS nebo Facebook. A v nejlepším případě je programování zaujme natolik, že po workshopu pokračují dále a v některém z dalších ročníků začnou samy koučovat.

Pokud vám poslední možnost přijde jen jako zbožné přání jak by to mělo fungovat, právě letos koučovaly dvě bývalé účastnice -- děvčata navíc chystají projekt [Bezlepci](http://www.bezlepci.cz/) pro lidi s bezlepkovou dietou. Rails Girls mají reálný dopad.

# Proč vůbec Ruby?

Když se bavím o Rails Girls s jinými vývojáři, nevyhnutelně dojde na otázku „Proč právě Ruby? Proč ne třeba `#{můj oblíbený jazyk}`?“ Akce se samozřejmě jmenuje *Rails* Girls, což implikuje framework [Ruby on Rails](http://rubyonrails.org/) -- a tím pádem i jazyk Ruby.

Osobně si však myslím, že volba Ruby není daná třeba syntaxí nebo jinými technickými vlastnostmi jazyka; přístupnost a otevřenost jsou spíše vlastnosti hluboko zakořeněné v komunitě Ruby vývojářů. Proč?

Začal bych u tvůrce jazyka Ruby -- Yukihiro „Matz“ Matsumoto. Není to sice geniální programátor, ale klade důraz na přístupnost programování pro lidi, nikoliv pro stroje. To bylo i principem pro návrh Ruby:
...
>
> <footer><cite>Yukihiro Matsumoto, přednáška [Ruby 1.9](https://www.youtube.com/watch?v=oEkJvvGEtB4) na Google tech Talks</cite>

Matzův nekonfliktní a přátelský přístup vedl i ke rčení <q lang="en">Matz is Nice So We Are Nice</q> jako určitému nepsanému pravidlu komunity. A přestože toto pravidlo [ztratilo časem váhu](http://blog.steveklabnik.com/posts/2011-08-19-matz-is-nice-so-we-are-nice) a [ne všichni rubysti](http://codon.com/the-dhh-problem) jsou tak přátelští, komunita kolem Ruby je rozmanitá a plná lidí, které prostě baví programovat.
...
<figure>
![díky Whyově srdceryvné příručce se i kojenci mohou naučit Ruby. Je to tak jednoduché!]({{page.assets}}/about.the.poignant.guide-2.jpg)
<figcaption>
...
<figcaption>
Linda Liukas: [Hello Ruby](http://kck.st/1hNiJag)
</figcaption>
</figure>

Linda chce ukázat svět softwaru jako svět netušených možností, svět, ve kterém se nápady snadno stanou skutečností -- a programování jako prostředek, jak se v tomto světě neztratit. Za zmínku stojí i fakt, že [projekt na Kickstarteru](http://kck.st/1hNiJag) byl zafinancován na 38-násobek původní částky.

Takže proto Ruby.

Ale můj pohled je samozřejmě zkreslený tím, že jsem rubysta. Jiné jazyky mají své vlastní iniciativy, ať už se jedná třeba o [pyLadies](http://www.pyladies.com/), [Clojure Bridge](http://www.clojurebridge.org/) nebo [PHP Women](http://phpwomen.org/). Cílem není pokračovat svatou válku o nejlepší jazyk, ale zpřístupnit programování všem, kteří mají aspoň trochu zájem, bez ohledu na rasu nebo pohlaví. <!-- Nedostatek diverzity v IT se týká celého odvětví, ne jenom Ruby. -->

# Proč koučovat?

Ačkoli jsem přednášel o programování v Ruby celý semestr na ČVUT, teprve Rails Girls byla skutečná prověrka mých znalostí. Část práce naštěstí odvedou přednášející -- Karel Minařík v úvodu vysvětlil, že „algoritmus“ je vlastně totéž, jako kuchařský recept. A Robin Pokorný zase ukázal, k čemu jsou dobré kaskádové styly. Ale je potom na koučích vysvětlit konkrétní koncepty a provést účastnice nejastnostmi. Po dvou dnech jsem necítil hlasivky a na nohou mě držel především kofein -- ale stálo to za to.

Atmosféra workshopu se nedá popsat; ještě jsem nezažil takový zájem o učení se, natož aby se jednalo o složité technické koncepty. Motivace účastnic dělá tyto workshopy skutečně výjimečné. A každé drobné vítězství nad kódem, každý úspěšně pochopený koncept je pro kouče ohromná radost.
...
Sobotní dopoledne věnujeme [TryRuby](http://tryruby.org/), které obsahuje krátký úvod do konceptů jazyka, provázeného whyovými ilustracemi. Ačkoli témata i texty jsou dobré, narazili jsme na několik konkrétních problémů s touto aplikací:

* Rychlost: aplikace běží kompletně on-line a odpovědi někdy trvají příliš dlouho, někdy nepřijdou vůbec.
* Nepředvidatelnost: ačkoli by se mělo jednat o plnohodnotný interpret jazyka, nechová se tak -- některé standardní metody zkrátka nefungují. Opačný problém je naopak v lekcích, kde se najednou pracuje s předdefinovanou proměnnou -- přitom její definice není vidět.
* Cíl: u lekcí jsou sice řečené dílčí kroky, ale není už příliš dobře vysvětlené, k čemu se směřuje. Je fajn, že si můžeme vypsat všechny prvky z pole, ale co s tím? Buď musí nastoupit koučové, nebo by měl být nejprve stanovený cíl a pak probrány dílčí kroky.
* Míra abstrakce: fakt, že aplikace běží v prohlížeči bez jakékoliv vazby na reálný stroj může být překvapivý problém. Když přijde na vypisování souborů, odkud se ten soubor vzal? Kde se nachází? V takovém případě je nejlepší vzít `irb` a ukázat tytéž příkazy na lokálním systému.

Bohužel stávající verze TryRuby je uzavřená, takže není v naší moci tyto problémy řešit v on-line verzi. Asi jako nejlepší alternativa se nabízí vytvořit gem s tutorialem, který poběží přes IRB přímo na stroji.
...
Třeba [Czechitas](http://www.czechitas.cz/), které pořádají Rails Girls na Moravě, berou algoritmizaci od základu v prostředí [Scratch](http://scratch.mit.edu/).

Nebo to můžeme vzít přes webové technologie. Dobrým příkladem je [Mozilla Webmaker](https://webmaker.org/), což je sada nástrojů a postupů jak začít tvořit na webu pro web. Koneckonců jsem si všiml, že účastnice zaujala možnost upravit existující stránky pomocí inspektoru, tedy standardního nástroje v každém prohlížeči -- což je zajímavá motivace pro pochopení webových technologií. Sám jsem se mnoho naučil psaním jednoduchých userscriptů pro [Greasemonkey](http://en.wikipedia.org/wiki/Greasemonkey) které upravovaly existující služby -- což je často přínosnější, než schopnost napsat si aplikaci vlastní.
