...
<i>Text vychází z přednášky na [květnovém Ruby srazu](http://pojd.me/3hg) v Praze.</i>
...
* Jenom to [nainstalovat](#howto) nemusí být sranda...

# Problém se slidy

Od loňského roku jsem začal na [Fakultě informačních technologií ČVUT](https://fit.cvut.cz/) učit programování v Ruby (předmět [MI-RUB](http://bk.fit.cvut.cz/cz/predmety/00/00/00/00/00/00/01/47/77/p1477706.html)). Jestli mě něco na přednáškách nebavilo, je to příprava materiálů. Bez ohledu na množství obrázků a jak moc to „zaobalíme“ dojde nevyhnutelně na ukázky kódu.

<figure class="full">
![]({{page.assets}}/slide.png)
</figure>

Prezentace pak slouží jednak jako vodítko pro přednášející, jednak jako reference pro studenty. Takže sáhneme po nějakém „slideware“ typu PowerPoint, případně po LaTeXu, který se alespoň postará o obarvení kódu. Progresivnější přednášející si připraví prezentaci v prohlížeči třeba s [Reveal.js](http://lab.hakim.se/reveal-js/), ještě lépe psanou v Markdown -- například s [Remark.js](http://remarkjs.com/). Ale ať obarvíme kód sebelépe, neřeší to jeden zásadní problém.
...
Pro účely výuky má však REPL řadu nevýhod. Není úplně jednoduché vrátit se zpět a změnit starší definici: musím si jí najít v historii, upravit a znovu spustit. Velký problém je i reprodukovatelnost: schválně, jak si v IRB uložím do souboru to, co jsem napsal? Že jsem měl použít [Pry](http://pryrepl.org/)? Výsledkem však bude stále jen neokomentovaný textový soubor s mými vstupy, který můžu spustit jako celek a nebo řádku po řádce. S přípravou materiálů to příliš nepomůže.
...
Zdá se, že vědci tento problém vyřešili už před více jak dvaceti roky. V aplikacích pro symbolickou matematiku a vizualizace, jako je Maple nebo Mathematica, se pracuje s interaktivními dokumenty, tzv. <i>worksheety</i> nebo <i>notebooky</i>.
...
Oddělení jádra od frontendu vypadá na první pohled jako samolibý <i>overengineering</i>, ale je to zcela zásadní výhoda IPythonu. Komunikace s jádrem probíhá pomocí [ZeroMQ](http://zeromq.org/), takže jádro může běžet na jiném stroji, než frontend. Jader ale může být víc, IPython má podporu pro [paralelní výpočty](http://ipython.org/ipython-doc/dev/parallel/parallel_intro.html). Můžete tak ovládat výpočetní cluster přímo z pohodlí notebooku.

<figure class="full">
![]({{page.assets}}/ipython2.png)
</figure>

Pro nás je ovšem zásadní fakt, že výpočetní jádro nemusí být vůbec napsané v Pythonu. Alternativní jádra mají stále výraznější podporu od tvůrců IPythonu. Velice dobré podpoře se těší [jádro pro Haskell](https://github.com/gibiansky/IHaskell); [jádro pro jazyk Julia](https://github.com/JuliaLang/IJulia.jl) je dokonce oficiální součástí [projektu Julia](http://julialang.org/#Designed.for.Parallelism.and.Cloud.Computing). A ano, existuje i jádro pro Ruby, obligátně nazvané [IRuby](https://github.com/minad/iruby).
...
IPython notebook je soubor s příponou `.ipynb`. Je to obyčejný [JSON](https://github.com/minad/iruby/blob/master/IRuby-Example.ipynb), ve kterém je jak obsah jednotlivých buněk, tak výstup spuštěného kódu. Přes příkaz `ipython notebook` (případně `iruby notebook`) si pak spustím v aktuálním adresáři webserver s editorem notebooku.
